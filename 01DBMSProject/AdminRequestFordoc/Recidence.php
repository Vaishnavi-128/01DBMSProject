...
<html>
  ...
  if (isset($_GET['error']) && $_GET['error'] === 'invalid_adhar') {
      echo '<script>alert("Please enter a valid 12-digit Aadhar Card number.");</script>';
  }
  ?>
	...
	<input type="hidden" name="form_id" value="f4">
      ...
      <input type="text" id="office" name="office" value="Veet" readonly required><br><br>
      <label for="taluka">Taluka(तालुका) : </label>
      <input type="text" id="taluka" name="taluka" value="Karmala" readonly required><br><br>
      <label for="District">District(जिल्हा) : </label>
      <input type="text" id="District" name="District"value="Solapur" readonly required><br><br>
       <label for="Date">Date(दिनांक) : </label>
      <input type="date" id="date" name="date" value="<?php echo date('Y-m-d'); ?>" readonly required><br><br>
