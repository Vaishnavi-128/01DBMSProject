...
$sql = "SELECT * FROM marriagecertificate where status = 'pending'";
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    echo "<tr>";
    echo "<td>" . $row["id"] . "</td>";
    echo "<td>" . $row["husbandName"] ." | ". $row["wifeName"] ."</td>";
    ...
    echo "<td><a href='EmailSender/emailWedding.php?id=" . $row["id"] . "' style='background-color: #28a745; color: #fff; padding: 5px 10px; border-radius: 4px; text-decoration: none;'>Sent</a></td>";
    echo "<td><a href='viewwedding.php?id=" . $row["id"] . "'>View Details</a></td>";
    echo "</tr>";
  }
} else {
  echo "0 results";
...
if(isset($_POST['update_status'])) {
    ...
    $sql = "UPDATE marriagecertificate SET status = 'done' WHERE id = $id";
