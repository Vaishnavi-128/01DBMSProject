...
$sql = "SELECT * FROM deathcertificate where status = 'pending' ";
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    echo "<td><a href='EmailSender/emailDeath.php?id=" . $row["id"] . "' style='background-color: #28a745; color: #fff; padding: 5px 10px; border-radius: 4px; text-decoration: none;'>Sent</a></td>";
    echo "<td><a href='viewdeath.php?id=" . $row["id"] . "'>View Details</a></td>";
    echo "</tr>";
  }
} else {
  echo "0 results";
...
if(isset($_POST['update_status'])) {
    ...
    $sql = "UPDATE deathcertificate SET status = 'done' WHERE id = $id";
