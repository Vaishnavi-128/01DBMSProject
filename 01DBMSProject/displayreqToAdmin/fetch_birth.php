...
$sql = "SELECT * FROM birthcertificate where status = 'pending' ";
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    echo "<td><a href='EmailSender/emailBirth.php?id=" . $row["id"] . "' style='background-color: #28a745; color: #fff; padding: 5px 10px; border-radius: 4px; text-decoration: none;'>Sent</a></td>";
    echo "<td><a href='view.php?id=" . $row["id"] . "'>View Details</a></td>";
    echo "</tr>";
  }
} else {
  echo "0 results";
...
if(isset($_POST['update_status'])) {
    ...
    $sql = "UPDATE birthcertificate SET status = 'done' WHERE id = $id";
